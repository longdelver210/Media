...
Item {
    ...
    PathView {
        ...
        onCurrentIndexChanged: {
            m_playlist.currentIndex = currentIndex
        }
    }
    //Progress
    Text {
    ...
    SwitchButton {
        ...
        status: m_playlist.playbackMode === Playlist.Random ? 1 : 0
        onClicked: {
            console.log(player.playlist.playbackMode)
            if (m_playlist.playbackMode === Playlist.Random) {
               m_playlist.playbackMode = Playlist.Sequential
            } else {
                m_playlist.playbackMode = Playlist.Random
            }
        }
    }
    ButtonControl {
    ...
    SwitchButton {
        ...
        status: m_playlist.playbackMode === Playlist.Loop ? 1 : 0
        onClicked: {
            console.log(m_playlist.playbackMode)
            if (m_playlist.playbackMode === Playlist.Loop) {
                m_playlist.playbackMode = Playlist.Sequential
            } else {
                m_playlist.playbackMode = Playlist.Loop
            }
        }
    }
